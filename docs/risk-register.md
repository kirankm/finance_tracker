...
| Linode production deployment exposes inbound SMS endpoint before HTTPS/auth hardening | Financial SMS data could be intercepted or submitted by unauthorized clients | Use Caddy for HTTPS, verify firewall rules, require a strong inbound secret, avoid raw SMS logs, and add contract tests before real ingestion | Open |
